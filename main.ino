...
// This EUI must be in LBS, so least-significant-byte
...
// This should also be in LBS, see above.
static const u1_t PROGMEM DEVEUI[8]= { FILL-ME-IN };
void os_getDevEui (u1_t* buf) { memcpy_P(buf, DEVEUI, 8);}

// This key should be in MBS (or, since it is not really a
...
// cycle limitations).
const unsigned SEND_INTERVAL = 60;
...
void onEvent (ev_t ev) {
    ...
    switch(ev) {
        ...
        case EV_JOINED:
            ...
            // Change to slower blink interval
            blink_interval = 5;
            break;
        case EV_RFU1:
...
void do_blink(osjob_t* j){
  ...
  // Schedule next transmission
  os_setTimedCallback(&blinkjob, os_getTime()+sec2osticks(blink_interval), do_blink);
}
